  ...
  switch (tipo) {
    case "confirm":
      alertify
        .confirm()
        .set("closable", false)
        .set("labels", { ok: btn_ok, cancel: btn_cancel })
        .setHeader(titular)
        .setContent(
          `
                <div>
...
function getFileHeader(file) {
  return new Promise((resolve) => {
    /* Read the first 4 bytes of the file */ var headerBytes = file.slice(0, 4);
...
function verifyType(bytePattern) {
...
function Ajax_CargadoGeneralPagina(
  ...
  try {
    ...
    $("#" + strDivError).empty();
    strdata =
      strdata + "&csrf_test_name=" + $("input[name=csrf_test_name]").val();
    $.post(strUrlDestino, strdata, function (data) {
      try {
        ...
        if (data.indexOf("Fatal error") >= 0) {
          $("#" + strDivError)
            .empty()
            .append(
              "<br />Existe un problema en la Generacion de la Pagina<br /><br />"
            );
          return;
        }
      } catch (e) {}
    ...
    $("#" + strDivError)
      .empty()
      .append("<br />Existe un problema en la Carga de la Pagina<br /><br />");
  }
}
